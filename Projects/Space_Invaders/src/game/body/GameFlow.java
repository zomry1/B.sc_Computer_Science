...
public class GameFlow {
    ...
    /**
     * Run the levels one after one (by the level number)
    ...
    public void runLevels() {
        ...
        this.score = new Counter();
        //Run until there is no more lives
        while (this.lives.getValue() > 0) {
            //Create a game level and initilaize it
            GameLevel level = new GameLevel(levelNumber, this.ar, this.ks, this.lives, this.score);
            level.initilaize();

            //while the user dont lose and there is still aliens on the screen
        ...
        this.ar.run(new KeyPressStoppableAnimation(this.ks, KeyboardSensor.SPACE_KEY,
                new EndScreen(false, this.score)));
